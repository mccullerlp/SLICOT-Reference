...
C
C     DE      (input/output) COMPLEX*20 array, dimension
C                            (LDDE, MIN(K+1,N))
...
C
C     B       (input/output) COMPLEX*20 array, dimension (LDB, K)
C             On entry, the leading N/2-by-N/2 part of this array must
...
C
C     FG      (input/output) COMPLEX*20 array, dimension
C                            (LDFG, MIN(K+1,N))
C             On entry, the leading N/2-by-N/2 lower triangular part of
C             this array must contain the lower triangular part of the
...
C     .. Array Arguments ..
      ...
      COMPLEX*20         A( LDA, * ), B( LDB, * ), DE( LDDE, * ),
     $                   FG( LDFG, * ), Q( LDQ, * ), ZWORK( * )
C
C     .. Local Scalars ..
