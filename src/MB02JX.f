...
C
C     Q       (output)  REAL*10 array, dimension (LDQ,RNK)
...
C             LDQ >= 1,           if JOB = 'R'.
C
C     R       (output)  REAL*10 array, dimension (LDR,RNK)
...
C     .. Scalar Arguments ..
     ...
     $                  RNK
      REAL*10  TOL1, TOL2
C     .. Array Arguments ..
      REAL*10  DWORK(LDWORK), Q(LDQ,*), R(LDR,*), TC(LDTC,*),
     $                  TR(LDTR,*)
      INTEGER           JPVT(*)
C     .. Local Scalars ..
      LOGICAL           COMPQ, LAST
