...
C
C     U1      (output) REAL*10 array, dimension (LDU1,N)
...
C             LDU1 >= N,    if JOBU = 'U'.
C
C     U2      (output) REAL*10 array, dimension (LDU2,N)
C             On exit, if JOBU = 'U', the leading N-by-N part of this
...
C             LDU2 >= N,    if JOBU = 'U'.
C
C     V1      (output) REAL*10 array, dimension (LDV1,N)
...
C             LDV1 >= N,    if JOBV = 'V'.
C
C     V2      (output) REAL*10 array, dimension (LDV2,N)
C             On exit, if JOBV = 'V', the leading N-by-N part of this
C             array contains the (2,1) block of the orthogonal
C             symplectic matrix V of decomposition (2).
C
...
C     .. Scalar Arguments ..
     ...
     $                   LDV2, LDWORK, N
C     .. Array Arguments ..
      REAL*10   A(LDA,*), DWORK(*), QG(LDQG,*), SCALE(*),
...
C     .. Local Scalars ..
     ...
     $                   PQ, PTAUL, PTAUR, PZ, WRKMIN, WRKOPT
      REAL*10   BIGNUM, CSCALE, EPS, HNR1, HNRM, SMLNUM, TEMP,
     $                   TEMPI, TEMPR
C     .. External Functions ..
      LOGICAL            LSAME
      REAL*10   DLAMCH, MA02ID
