...
C     Workspace
C
C     DWORK   REAL*16 array, dimension (8*K)
C
C     Error Indicator
C
C     INFO    INTEGER
...
C     .. Scalar Arguments ..
      ...
      REAL*16  BASE, LGBAS, ULP
C     .. Array Arguments ..
      REAL*16  A(LDA1,LDA2,*), ALPHAI(2), ALPHAR(2), BETA(2),
     $                  DWORK(*)
      INTEGER           AMAP(*), S(*), SCAL(2)
C     .. Local Scalars ..
      INTEGER           AI, I, IITER, J, PDM, PDW, SL
      REAL*16  CS, CST, LHS, MISC, MISR, RHS, TEMPI, TEMPR
      COMPLEX*32        SN, SNT, TEMP
C     .. Local Arrays ..
      COMPLEX*32        T(2,2), Z(3,3)
