...
C
C     A       (input/workspace) COMPLEX*32 array, dimension (LDA,N)
...
C
C     E       (input/workspace) COMPLEX*32 array, dimension (LDE,N)
C             The leading N-by-N upper triangular part of this array
...
C
C     ZWORK   COMPLEX*32, dimension MAX(3*N-3,0)
C
C     Error indicator
C
C     INFO    INTEGER
...
C     .. Array Arguments ..
      REAL*16  DWORK(*)
      COMPLEX*32        A(LDA,*), B(LDB,*), E(LDE,*), ZWORK(*)
C     .. Local Scalars ..
      COMPLEX*32        M1, R, S, X, Z
      REAL*16  BIGNUM, C, DELTA1, EPS, M2, SCALE1, SMLNUM, T,
     $                  UII
      INTEGER           APT, I, J, KL, KL1, UPT, WPT
      LOGICAL           NOTRNS
C     .. Local Arrays ..
      COMPLEX*32        M3(2,2), M3C(2,1)
      REAL*16  D(2), ES(2), W(2)
      INTEGER           IWORK(7)
