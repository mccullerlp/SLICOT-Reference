...
C     .. External Subroutines ..
      EXTERNAL           DAXPY, DCOPY, DGEES, DGEMM, DGESV, DLACN2,
...
C
     ...
     $                     INFO )
               WRKOPT = MAX( LDW, INT( DWORK( 1 ) ) + LWA + 2*N )
            ELSE
               WRKOPT = LDW
            END IF
         END IF
         IF( LDWORK.LT.LDW .AND. .NOT. LQUERY )
     $      INFO = -24
      END IF
C
      IF( INFO.NE.0 ) THEN
...
C        REPEAT
   30    CONTINUE
         CALL DLACN2( NN, DWORK( IXBS+1 ), DWORK, IWORK, EST, KASE,
     $                ISAVE )
         IF( KASE.NE.0 ) THEN
C
