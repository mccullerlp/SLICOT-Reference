...
C
C     UL      (input/workspace) REAL*10 array, dimension
C             ( LDUL,L*NOBR )
...
C
C     R1      (input) REAL*10 array, dimension ( LDR1,N )
...
C             LDR1 >= 1,          otherwise.
C
C     TAU1    (input) REAL*10 array, dimension ( N )
C             If  JOB <> 'N',  M > 0,  METH = 'M',  and  RANKR1 = N,
...
C
C     K       (input/output) REAL*10 array, dimension
C             ( LDK,M*NOBR )
...
C
C     R       (output) REAL*10 array, dimension ( LDR,L*NOBR )
...
C
C     H       (output) REAL*10 array, dimension ( LDH,M )
...
C     .. Scalar Arguments ..
     ...
     $                   LDR, LDR1, LDUL, LDWORK, M, N, NOBR, RANKR1
      CHARACTER          JOB, METH
C     .. Array Arguments ..
      REAL*10   B(LDB, *), D(LDD, *), DWORK(*), H(LDH, *),
     $                   K(LDK, *), PGAL(LDPGAL, *), R(LDR, *),
     $                   R1(LDR1, *), TAU1(*), UL(LDUL, *)
      INTEGER            IWORK( * )
C     .. Local Scalars ..
      REAL*10   EPS, RCOND, SVLMAX, THRESH, TOLL
