...
C
C     DE      (input/output) COMPLEX*20 array, dimension (LDDE, N+1)
...
C
C     VW      (input/output) COMPLEX*20 array, dimension (LDVW, N+1)
C             On entry, the leading N-by-N lower triangular part of
C             this array must contain the lower triangular part of the
...
C
C     LSCALE  (output) REAL*10 array, dimension (N)
...
C     .. Scalar Arguments ..
      ...
      REAL*10   THRESH
C     .. Array Arguments ..
      REAL*10   DWORK(*), LSCALE(*), RSCALE(*)
      COMPLEX*20         A(LDA,*), C(LDC,*), DE(LDDE,*), VW(LDVW,*)
C     .. Local Scalars ..
     ...
     $                   KW7, LRAB, LSFMAX, LSFMIN, NR, NRP2
      REAL*10   AB, ALPHA, BASL, BETA, CAB, CMAX, COEF, COEF2,
