...
C
C     EX      (output) REAL*10 array, dimension (LDEX,N)
...
C
C     EXINT   (output) REAL*10 array, dimension (LDEXIN,N)
C             The leading N-by-N part of this array contains an
...
C
C     .. Parameters ..
      REAL*10  ZERO, HALF, ONE, ONE64, THREE, FOUR8
...
C     .. Array Arguments ..
      INTEGER           IWORK(*)
      REAL*10  A(LDA,*), DWORK(*), EX(LDEX,*), EXINT(LDEXIN,*)
C     .. Local Scalars ..
      INTEGER           I, I2IQ1, IJ, IQ, J, JSCAL, KK, L, NN
      REAL*10  COEFFD, COEFFN, DELSC, EPS, ERR, F2IQ1,
