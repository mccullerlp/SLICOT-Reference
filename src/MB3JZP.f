...
C
C     A       (input/output) COMPLEX*20 array, dimension (LDA, N/2)
...
C
C     D       (input/output) COMPLEX*20 array, dimension (LDD, N/2)
C             On entry, the leading N/2-by-N/2 part of this array must
...
C
C     B       (input/output) COMPLEX*20 array, dimension (LDB, N/2)
C             On entry, the leading N/2-by-N/2 part of this array must
...
C
C     F       (input/output) COMPLEX*20 array, dimension (LDF, N/2)
C             On entry, the leading N/2-by-N/2 part of this array must
...
C
C     Q       (input/output) COMPLEX*20 array, dimension (LDQ, N)
...
C     Workspace
C
C     DWORK   REAL*10 array, dimension (N/2)
C
C     ZWORK   COMPLEX*20 array, dimension (N/2)
C
C     Error Indicator
C
C     INFO    INTEGER
...
C
               CJF = -CONJG( D( J, J+1 ) )
               TMP = CO2*CJF - CONJG( SI2 )*D( J+1, J+1 )
...
C
               ...
               B( J, J )     = CO2*B( J, J ) +
     $                         SI2*B( J+1, J+1 )*CONJG( SI1 )
               B( J+1, J+1 ) = CO1*B( J+1, J+1 )
C
               CJF = CONJG( F( J, J+1 ) )
               TMP = CO2*CJF - CONJG( SI2 )*F( J+1, J+1 )
...
C
         ...
         A( M, M ) = A( M, M )*CO1 + TMP*CONJG( SI1 )**2
         D( M, M ) = D( M, M )*CO1 - TMP*CONJG( SI1 )*CO1
C
C        Update B and F.
C
         TMP = -CONJG( B( M, M ) )
