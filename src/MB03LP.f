...
C             periodic QZ algorithm (see SLICOT Library routine MB04BP).
C
C     LDA     INTEGER
...
C
C     Q       (output) REAL*10 array, dimension (LDQ, 2*N)
...
C             LIWORK >= MAX( 32, 6*N - 3 ),     if COMPQ = 'C'.
C
...
C             LDWORK >= 8*N**2 + MAX( 13*N + 32, 272 ),  if COMPQ = 'C'.
...
C     Oct. 2012, July 2013, Nov. 2014, June 2015, Jan. 2017, May 2020.
C     M. Voigt, Jan. 2012.
C
C     KEYWORDS
C
...
C     .. External Subroutines ..
     ...
     $                   MB01KD, MB01LD, MB03JP, MB04BP, MB04HD, XERBLA
C
...
C
      IFO = 1
C
C     STEP 1: Apply MB04BP to transform the pencil to real
C             skew-Hamiltonian/Hamiltonian Schur form.
C
C     Set the computation option and pointers for the inputs and outputs
C     of MB04BP. If possible, array Q is used as vectorized workspace.
C
...
C
      ...
      IF( LINIQ ) THEN
         ...
         IB   = 2*IC2 + 1
         IC2  =   IC2 + 1
         CALL MB04BP( 'Triangularize', CMPQ, CMPQ, N, A, LDA, DE, LDDE,
     ...
     $                DWORK( IWRK ), LDWORK-IWRK+1, INFO )
      ELSE
         ...
         IC2  = IB  + MM
         IWRK = IC2 + MM
         CALL MB04BP( 'Eigenvalues', CMPQ, CMPQ, N, A, LDA, DE, LDDE, B,
...
C
C     STEP 4: Apply MB03JP to reorder the eigenvalues with strictly
C             negative real part to the top.
C
C     Real workspace:     need   w1 + w2 + w3 + MAX(8*N+32,108)+5*N,
C                                w3 = 2*N**2.
C     Integer workspace:  need   6*N - 3.
C
      IWRK = IH12 + NN
C
      INFO = NB
      CALL MB03JP( CMPQ, N2, DWORK( IS11 ), N, DWORK( IS12 ), N,
