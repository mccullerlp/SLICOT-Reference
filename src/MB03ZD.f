...
C
C     S       (input/output) REAL*10 array, dimension (LDS,N)
...
C
C     V2      (input/output) REAL*10 array, dimension (LDV1,N)
C             On entry, the leading N-by-N part of this array must
C             contain the (2,1) block of an orthogonal symplectic
C             matrix V.
C             On exit, this array is overwritten.
...
C
C     US      (output) REAL*10 array, dimension (LDUS,MM)
...
C
C     UU      (output) REAL*10 array, dimension (LDUU,MM)
...
C     .. Array Arguments ..
      ...
      INTEGER           IWORK(*)
      REAL*10  DWORK(*), G(LDG,*), S(LDS,*), SCALE(*),
...
C     .. Local Scalars ..
     ...
     $                  PAIR
      ...
      REAL*10  NRMIN, RCOND, TEMP, TOL
C     .. External Functions ..
      LOGICAL           LSAME
      REAL*10  DASUM, DLAMCH, DLANGE
