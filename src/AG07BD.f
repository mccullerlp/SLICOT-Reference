...
C
C     AI      (output) REAL*10 array, dimension (LDAI,N+M)
...
C             LDAI >= MAX(1,N+M).
C
C     EI      (output) REAL*10 array, dimension (LDEI,N+M)
C             The leading (N+M)-by-(N+M) part of this array contains
...
C             LDEI >= MAX(1,N+M).
C
C     BI      (output) REAL*10 array, dimension (LDBI,M)
...
C             LDBI >= MAX(1,N+M).
C
C     CI      (output) REAL*10 array, dimension (LDCI,N+M)
...
C
C     DI      (output) REAL*10 array, dimension (LDDI,M)
...
C     .. Array Arguments ..
      REAL*10   A(LDA,*), AI(LDAI,*), B(LDB,*), BI(LDBI,*),
