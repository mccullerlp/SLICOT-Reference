...
C     computed by the SLICOT Library routine MB04DP, to the matrix
C
C               [   V1   ]
C               [        ],
C               [ sgn*V2 ]
...
C
C     ILO     (input) INTEGER
C             The integer ILO determined by MB04DP.
C             1 <= ILO <= N+1.
C
C     LSCALE  (input) REAL*16 array, dimension (N)
C             Details of the permutation and scaling factors applied
C             from the left, as returned by MB04DP.
C
C     RSCALE  (input) REAL*16 array, dimension (N)
C             Details of the permutation and scaling factors applied
C             from the right, as returned by MB04DP.
C
C     M       (input) INTEGER
...
C     .. Array Arguments ..
      REAL*16  LSCALE(*), RSCALE(*), V1(LDV1,*), V2(LDV2,*)
C     .. Local Scalars ..
...
C
      IF ( LSCAL ) THEN
         DO 10 I = ILO, N
            CALL DRSCL( M, LSCALE(I), V1(I,1), LDV1 )
   10    CONTINUE
         DO 20 I = ILO, N
            CALL DRSCL( M, RSCALE(I), V2(I,1), LDV2 )
   20    CONTINUE
      END IF
C
