...
C             1 <= ILO <= M+1.
C
C     V       (input) REAL*10 array, dimension (LDV,M)
...
C
C     W       (input) REAL*10 array, dimension (LDW,M)
C             On entry, the leading M-by-M part of this array must
C             contain in its columns the vectors which define the
...
C
C     C       (input/output) REAL*10 array, dimension
C                     (LDC,N) if TRANC = 'N',
...
C
C     CS      (input) REAL*10 array, dimension (2*N-2)
...
C     .. Array Arguments ..
      REAL*10  C(LDC,*), CS(*), D(LDD,*), DWORK(*), TAU(*),
     $                  V(LDV,*), W(LDW,*)
C     .. Local Scalars ..
