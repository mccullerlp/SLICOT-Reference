...
C
C     A       (input/output) COMPLEX*20 array, dimension (LDA,M)
...
C
C     B       (input/output) COMPLEX*20 array, dimension (LDB,L)
...
C             LDB >= 1        if L = 0.
C
C     TAU     (output) COMPLEX*20 array, dimension MIN(N,M)
...
C     .. Array Arguments ..
      COMPLEX*20        A(LDA,*), B(LDB,*), TAU(*), ZWORK(*)
C     .. Local Scalars ..
      LOGICAL           LQUERY
      INTEGER           I, WRKOPT
      COMPLEX*20        FIRST
