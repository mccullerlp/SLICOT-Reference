...
C             consecutively REAL*10 vectors mu, delta, gamma,
...
C             contain L REAL*10 values, and gamma and kappa
C             must each contain L-1 REAL*10 values.
...
C
C     G       (output) REAL*10 array, dimension (NG)
...
C
C     X       (output) REAL*10 array, dimension (LDX,IPAR(1))
...
C     .. Local Scalars ..
     ...
     1                 PSYMM, QDIMM
      REAL*10 APPIND, B1, B2, C1, C2, SUM, TEMP, TTEMP
C
C     ..Local Arrays ..
      INTEGER          MDEF(2,NMAX), NDEF(4,NMAX), NEX(4), PDEF(2,NMAX)
      REAL*10 PARDEF(4,NMAX)
      CHARACTER        IDENT*4
